...
@Component({
  standalone: true,
  selector: 'app-print-customer-payment',
  imports: [CommonModule],
  template: `
    <main class="ticket" *ngIf="data">
      <h1>{{ data.branding?.displayName }}</h1>
      <p>Pago cuenta #{{ data.paymentId }}</p>
      <p>Cliente: {{ data.customerName }}</p>
      <p>{{ data.createdAt | date:'short' }}</p>
      <hr />
      <div class="row"><span>Monto</span><strong>{{ data.amount | number:'1.2-2' }}</strong></div>
      <div class="row"><span>Tipo</span><span>{{ movementTypeLabel(data.movementType) }}</span></div>
      <div class="row" *ngIf="data.reference"><span>Referencia</span><span>{{ data.reference }}</span></div>
...
export class PrintCustomerPaymentComponent {
  data: any;
  ...
  async load(id: number, reprint: boolean, autoPrint: boolean): Promise<void> {
    this.data = await this.api.customerPayment(id, reprint);
