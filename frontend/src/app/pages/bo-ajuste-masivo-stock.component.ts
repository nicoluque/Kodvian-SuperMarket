...
@Component({
  standalone: true,
  selector: 'app-bo-ajuste-masivo-stock',
  ...
  template: `
    <main class="wrap">
      ...
      <section class="content">
        <header class="hero">
          <div class="hero-copy">
            <h1>Ajuste masivo de stock</h1>
            <p>Proceso post inventario para sumar o restar cantidades sin permitir stock final negativo.</p>
          </div>
          <div class="hero-side">
            <span class="hero-pill">Modo estricto: si hay errores no se aplica nada</span>
            <a class="hero-link" routerLink="/bo/importaciones/unificada">Ir a importación unificada</a>
          </div>
        </header>
        ...
        <section class="card flow-card">
          <div class="flow-head">
            ...
            <p>Descargá la plantilla, cargá el archivo, validá resultados y confirmá para aplicar ajustes.</p>
          </div>

          <div class="toolbar-grid">
            <button class="btn btn-secondary" (click)="downloadTemplate()" [disabled]="loadingTemplate">
              {{ loadingTemplate ? 'Descargando...' : '1) Descargar plantilla de ajustes' }}
            </button>
          ...
          <div class="flow-footer">
            ...
            <ng-template #noFileMeta>
              <p class="meta">Subí un archivo <strong>.xlsx</strong> para validar ajustes.</p>
            </ng-template>
          </div>
        </section>
        ...
        <section class="card" *ngIf="preview">
          <div class="section-title">
            ...
            <span class="subtitle">Ajuste masivo</span>
          </div>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Estado</th>
                  <th>Producto</th>
                  <th>Delta vendible</th>
                  <th>Resultante vendible</th>
                  <th>Delta reclamo</th>
                  <th>Resultante reclamo</th>
                  <th>Delta merma</th>
                  <th>Resultante merma</th>
                  <th>Detalle</th>
                </tr>
              </thead>
              <tbody>
                <tr *ngFor="let r of filteredRows" [class.invalid-row]="!r.valid">
                  ...
                  <td>{{ r.data?.productNameResolved || r.data?.productName || '-' }}</td>
                  <td>{{ r.data?.vendibleDelta || '0' }}</td>
                  <td>{{ r.data?.resultVendibleQty || '-' }}</td>
                  <td>{{ r.data?.reclamoDelta || '0' }}</td>
                  <td>{{ r.data?.resultReclamoQty || '-' }}</td>
                  <td>{{ r.data?.mermaDelta || '0' }}</td>
                  <td>{{ r.data?.resultMermaQty || '-' }}</td>
                  <td>
        ...
        <section class="card" *ngIf="commit">
          ...
          <div class="kpis">
            <div class="kpi"><span>Ajustes aplicados</span><strong>{{ commit.adjustmentsApplied }}</strong></div>
          </div>
        </section>

        <section class="card" *ngIf="!preview && !commit && !loading && !error">
          <h3>Sin ajuste ejecutado</h3>
          <ol class="steps">
            <li>Descargá la plantilla de ajustes.</li>
            <li>Completá deltas positivos o negativos por artículo.</li>
            <li>Subí el archivo y validá la vista previa.</li>
            <li>Confirmá para aplicar cambios sin dejar stock final negativo.</li>
          </ol>
        </section>
      </section>
    </main>
  ...
  styles: [
    ...
    `table{width:100%;border-collapse:collapse;min-width:1120px;background:#fff;border-radius:12px;overflow:hidden}`,
    ...
    `@media (max-width: 900px){.wrap{padding:16px}.hero-side{align-items:flex-start;width:100%}.toolbar-grid{grid-template-columns:1fr}.btn,.file-drop{width:100%}.preview-tools{flex-direction:column;align-items:flex-start}}`
  ]
})
export class BoAjusteMasivoStockComponent {
  file: File | null = null;
  preview: any = null;
  commit: any = null;
  loading = false;
  ...
  fieldLabel(field: string): string {
    const map: Record<string, string> = {
      vendibleDelta: 'Delta vendible',
      reclamoDelta: 'Delta reclamo',
      mermaDelta: 'Delta merma',
  ...
  async downloadTemplate(): Promise<void> {
    ...
    this.error = '';
    try {
      await this.imports.downloadStockAdjustmentsTemplate();
    } catch (err: any) {
  ...
  async previewFile(): Promise<void> {
    ...
    this.commit = null;
    try {
      this.preview = await this.imports.stockAdjustmentsPreview(this.file);
    } catch (err: any) {
  ...
  async commitFile(): Promise<void> {
    ...
    this.error = '';
    try {
      this.commit = await this.imports.stockAdjustmentsCommit(this.file);
    } catch (err: any) {
      this.error = err?.error?.message ?? 'No se pudo confirmar ajuste masivo';
